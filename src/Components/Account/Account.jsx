import React, { useEffect, useState } from "react";
import "./account.css";
import { db,auth } from "../../firebase/config";
import { collection, where, getDocs, query,deleteDoc,doc } from "firebase/firestore";
import { useLocation ,useNavigate} from "react-router-dom";
import { sendPasswordResetEmail } from "firebase/auth";

function Account() {
  const location = useLocation();
  const navigate = useNavigate();
  const userID = location.state;//useNavigate vazzi pass cheytha data edukkan
  const [product, setProduct] = useState([]);
  const [productID, setProductID] = useState(null);
  const [user, setUser] = useState(null);
  useEffect(() => {
    async function getDocument() {
      const q = query(
        collection(db, "users"),
        where("userID", "==", userID) //userid post.userid aya data edukkum
      );

      const querySnapshot = await getDocs(q);
      querySnapshot.forEach((doc) => {
        // doc.data() is never undefined for query doc snapshots
        const data = doc.data();
        setUser(data);
      });
    }
    getDocument();
    async function getData() {
      const q = query(
        collection(db, "products"),
        where("userID", "==", userID) //userid post.userid aya data edukkum
      );

      const querySnapshot = await getDocs(q);
      const products = querySnapshot.docs.map((doc) => ({
        ...doc.data(),
        id: doc.id,
      }));

      setProduct(products)
    }
    getData();
  }, []);


 async function productDelete(productID){
      await deleteDoc(doc(db, "products", productID));
      alert('delete Compleat')
  }

  function resetPAssword(){
    sendPasswordResetEmail(auth, user.email)
  .then(() => {
 alert('send mail in reset password')
  })
  .catch((error) => {
    const errorCode = error.code;
    const errorMessage = error.message;
    console.log(error);
    // ..
  });
  }
 async function editProduct (productId){
       await navigate('/editPrdoct' , {state :productId})
  }

  return (
    <div className="accountParent">

      <div className="accountDetails">
        <img
          src="data:image/png;base64,"
          className="userImage"
          alt=""
        />
        <h1 className="userName">{user ? user.username : ""}</h1>
        <p className="email">Email : {user ? user.email : ""}</p>
        <p className="phone">Phone : {user ? user.phoneNo : ""}</p>
        <button onClick={()=>{resetPAssword()}} className="changePasswordBtn">Change Password</button>
        <button className="changePasswordBtn">Edit Profile</button>
      </div>
      <div className="accountProducts">
        {product.map((pro,i) => {
          //console.log(pro.id);
            return(
              <div className="card"
              key={i}>
              <div className="imageParent" key={i}>
                <img
                  src={pro.image[0]}
                  alt=""
                  className="postImage"
                />
              </div>
      
              <h1>{pro.price}</h1>
              <p className="shortDetail">{pro.shortDetail}</p>
              <p className="name">{pro.name}</p>
              <div className="bottomDiv">
                <p className="location">trivandrum</p>
                <p className="date">{pro.createdAt}</p>
              </div>
              <button onClick={()=>{productDelete(pro.id)}} >Delete</button>
              <button onClick={()=>{editProduct(pro.id)}} style={{margin:'10px'}}>Edit</button>
            </div>
            )
        })}
       
      </div>
    </div>
  );
}

export default Account;
